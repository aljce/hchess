...
gIndexGen :: (Index -> Index :*: Word64) -> MoveData -> Word64 -> Moves 
gIndexGen movement md = concatMap (uncurrySP (serializeBitBoard md) . movement) .
  expandBitBoard countTrailingZeros
...
knightMovement Black (AllColors bn _ _) (AllColors ba _ _) = gIndexGen movement KnightM bn
        where movement i = Pair i (complement ba .&. knightAttack ! i)
knightMovement White (AllColors _ wn _) (AllColors _ wa _) = gIndexGen movement KnightM wn
        where movement i = Pair i (complement wa .&. knightAttack ! i)
